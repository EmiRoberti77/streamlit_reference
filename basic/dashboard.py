...
with st.form("Population-form"):
  ...
  with col1:
    ...
    start_year = st.slider("Year", min_value=1991, max_value=2023, value=2021, step=1, key="start_y")
...
def display_dashboard(start_date, end_date, target):
  ...
  with tab1:
    st.subheader(f"Population change from {start_date} {end_date}")
    ...
    with col1:
      ...
      percentage_dif = round((final - initial) / initial * 100, 2)
      delta = f"{percentage_dif}%"
      st.metric(start_date, value=initial)
      st.metric(end_date, value=final, delta=delta)
    
    with col2:
      start_index = df.loc[df["Quarter"] == start_date].index.item()
      end_idx = df.loc[df["Quarter"] == end_date].index.item()

      filtered_df = df.iloc[start_index:end_idx+1]
      fig, ax = plt.subplots()
